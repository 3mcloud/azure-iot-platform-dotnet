...
export class ContextMenuAgile extends Component {
    ...
    updateMenu(priorityChildren) {
        ...
        const arrayAmount = this.howManyItemsInMenuArray(
            childrenWidths,
            containerWidth,
            moreMenuWidth,
            1
        );
        const navItemsCopy = priorityChildren;
        const priorityChildrenArray = navItemsCopy.slice(0, arrayAmount);
        const moreChildrenArray =
            priorityChildrenArray.length !== navItemsCopy.length
